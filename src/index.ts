...
const defaults: MiddlewareParams = {
  ...
  devLogger: async ({ req, res, e, name }) => {
    ...
    if (e) {
      console.error(
        `${method} ${url} => ${status}: An unhandled exception was caught inside ${name}: ${e}`
      )
    } else {
      ...
      await handler(req, res, end)
        .then(() => {
          if (process.env.NODE_ENV !== 'production') {
            console.log(
              `${req.method} ${req.url} => ${res.statusCode}: Don't forget to call end() after sending a response`
            )
          }
          end()
        })
        .catch((e) => {
          ...
          this.params
            .logger({ req, res, name, e: unHandledError })
            .catch((err) => {
              console.error(`${this.params.name}-logger: ${err}`)
            })
          ...
          if (process.env.NODE_ENV !== 'production') {
            this.params
              .devLogger({ req, res, name, e: unHandledError })
              .catch((err) => {
                console.error(`${this.params.name}-devLogger: ${err}`)
              })
          }
        })
    }
